		...
		if(!event.getWorld().isRemote) { 	
			if(player != null) {
				...
				if(event.getState().getBlock().equals(Blocks.GRAVEL)) {
					event.getDrops().clear();
					event.getDrops().add(new ItemStack(RegItems.stonehand,event.getWorld().rand.nextInt(3)));
				}
				...
				if(event.getState().getBlock().equals(Blocks.STONE)) {
					event.getDrops().clear();
					event.getDrops().add(new ItemStack(RegItems.stonehand, 1 + event.getWorld().rand.nextInt(5)));
				}
